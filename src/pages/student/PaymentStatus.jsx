...
export default function PaymentStatus() {
  return (
    <div>
      <div className="mb-8">
        <h1 className="font-display text-3xl font-bold text-gray-900">Payment Status</h1>
      ...
      <div className="grid grid-cols-3 gap-5 mb-8">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          ...
          <p className="text-xs text-gray-400 mt-1 flex items-center gap-1">
            <CheckCircle size={11} className="text-green-500" /> 4 Payments Completed
          </p>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
          <p className="text-sm text-gray-500 mb-1">Payment Method</p>
          <p className="font-display text-xl font-bold text-gray-900">Online & Cash</p>
          <p className="text-xs text-gray-400 mt-1 flex items-center gap-1">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-50">
          <h2 className="font-display font-bold text-gray-900">Payments History</h2>
        </div>
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-50">
              {['Order ID', 'Date', 'Amount', 'Status'].map(h => (
                <th key={h} className="px-6 py-3 text-xs font-medium text-gray-400 text-left">{h}</th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {payments.map((p, i) => (
              <tr key={i} className={`hover:bg-gray-50 transition-colors ${i === 0 ? 'bg-blue-50' : ''}`}>
                <td className="px-6 py-3.5 text-sm text-blue-600 font-medium">{p.id}</td>
                <td className="px-6 py-3.5 text-sm text-gray-600">{p.date}</td>
                <td className="px-6 py-3.5 text-sm text-gray-800">{p.amount}</td>
                <td className="px-6 py-3.5">
                  <span className={`text-sm font-medium ${p.status === 'Success' ? 'text-green-500' : 'text-orange-500'}`}>
                    {p.status}
                  </span>
                </td>
              </tr>
